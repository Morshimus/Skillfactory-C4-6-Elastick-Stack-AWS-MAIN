...
function init {
    ...
    }elseif($cloud_provider -like "aws"){
        $path = (Get-Location).path -replace "\\", "\\"
        $state = Get-Content  "$path\\..\\aws-s3-bucket-morsh\\terraform.tfstate"
        $table = $state | ConvertFrom-Json
       
        $S3_NAME = $table.outputs.name.value
        $S3_REGION = $table.outputs.region.value
