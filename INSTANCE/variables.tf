...
variable "image_regex" {
  type        = string
  description = <<-EOT
    (Optional) Regex string to apply to the AMI list returned by AWS. 
    ...
    Combine this with other options to narrow down the list AWS returns.
    EOT
  default     = null
}
...
variable "image_name" {
  type        = list(string)
  description = "Filtered value for finding fitting image, asterisks signs are accepted"

}

variable "image_root_dev_type" {
  type        = list(string)
  description = "Filtered value for finding fitting image root device type."
  default     = ["ebs"]
}


variable "image_virt_type" {
  type        = list(string)
  description = "Filtered valu for finding fitting umage virtualization type."
  default     = ["hvm"]
...
variable "image_owners" {
  type        = list(string)
  description = <<-EOT
    (Optional) List of AMI owners to limit search. Valid values: an AWS account ID, 
    self (the current account), or an AWS owner alias (e.g., amazon, aws-marketplace, microsoft).
    EOT
  default     = []
}

variable "image_executable_users" {
  type        = list(string)
  description = <<-EOT
